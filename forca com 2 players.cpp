...
void jogadorUM() {
	...
	int tentativas = 5, acertou = 0, tamanhopalavra = 0, erros = 0, i, contadorvitorias = 0;
	char tela [50];
	Palavras p;
	Ranking r;
  	FILE *arq;
...
void jogadorDOIS() {
	...
	fscanf(pontuacao, "%d", r.pontosdois);
//	fscanf(pontuacao, "%d", r.pontosum);
...
void ranking() { 
	...
	pontuacao = fopen("ranking.txt", "a+");
	fprintf(pontuacao, "\n\tPONTUAÇÃO DO JOGADOR 1 NA %d PARTIDA:\t\t%d", r.pontosum, r.partida);
	fprintf(pontuacao, "\n\tPONTUAÇÃO DO JOGADOR 2 NA %d PARTIDA::\t\t%d", r.pontosdois, r.partida);
  	fclose(pontuacao);
	...
	if(r.pontosum > r.pontosdois) { //primeiro jogador na frente
		...
		printf("\n\n\tSEGUNDO LUGAR:\t\tJOGADOR 2!\n");
		printf("\n\n\tPOSSUI: %d VITÓRIAS\n\n", r.pontosdois);
	} else if(r.pontosdois > r.pontosum) { //segundo jogador na frente
		
		printf("\n\n\tPRIMEIRO LUGAR:\t\tJOGADOR 2!\n");
		printf("\n\n\tPOSSUI: %d VITÓRIAS\n\n", r.pontosum);
		...
		printf("\n\n\tPOSSUI: %d VITÓRIAS\n\n", r.pontosdois);
	} else if(r.pontosdois == r.pontosum) { //empate
	
		printf("\n\n\tTEMOS UM EMPATE!\n\n\tOS JOGADORES 1 E 2 TÊM A MESMA PONTUAÇÃO ----> %d :O\n\n\tQUE TAL UM DESEMPATE?", r.pontosdois);
	}
